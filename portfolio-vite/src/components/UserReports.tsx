...
const reports = [
  {
    ...
    report: 'Cleanest code I’ve seen in years. He knows how to handle complex state management.',
    rating: 3,
    color: 'pink',
  },
  {
  ...
  },
  {
    ...
    report: 'Highly intuitive UX. Delivered exactly what we needed before we even knew we needed it.',
    rating: 5,
    color: 'orange',
  },
  {
    ...
    <section className="py-0 px-0 bg-[#18191c] flex flex-col items-center min-h-[600px] w-full">
      ...
      <div className="w-full flex justify-center pt-16 pb-10">
        <div className="relative bg-[#23242a] border-b-4 border-blue-700 rounded-md shadow-lg px-8 py-4 flex items-center min-w-[420px] max-w-[520px]">
          ...
          <span className="ml-16 font-mono text-2xl text-white font-extrabold tracking-wider">USER_REPORTS.txt</span>
          <span className="ml-6 px-3 py-1 bg-blue-600 text-xs font-bold text-white rounded shadow border border-blue-800">LIVE_FEED</span>
        </div>
      </div>
      {/* Marquee cards */}
        ...
        >
          {reports.concat(reports).map((report, idx) => (
            <div
              ...
              style={{ transition: 'transform 0.2s', borderTop: `4px solid var(--tw-${report.color}-500, ${report.color})` }}
            >
              {/* Top bar */}
              <div className="flex justify-between items-center px-5 pt-3 pb-1">
                <span className={`font-mono text-xs font-bold tracking-widest ${report.color === 'pink' ? 'text-pink-400' : report.color === 'purple' ? 'text-purple-400' : report.color === 'orange' ? 'text-yellow-400' : 'text-green-400'}`}>{report.log}</span>
                <span className="text-xs text-gray-400 font-mono">{report.file}</span>
              </div>
              <div className="px-5 text-xs text-gray-400 font-mono mb-1">FROM: {report.from}</div>
              <div className="px-5 text-xl font-extrabold text-white leading-snug mb-2">”{report.report}”</div>
              <div className="flex items-center px-5 pb-3">
                {Array.from({ length: report.rating }).map((_, i) => (
                  <span key={i} className={`text-lg mr-1 ${report.color === 'pink' ? 'text-pink-400' : report.color === 'purple' ? 'text-purple-400' : report.color === 'orange' ? 'text-yellow-400' : 'text-green-400'}`}>★</span>
                ))}
              </div>
              {/* Corner fold */}
              <span className="absolute top-0 right-0 text-[10px] text-gray-400 font-mono bg-[#23242a] px-2 py-0.5 rounded-bl-md border-b-2 border-l-2 border-black">{report.file}</span>
            </div>
          ))}
        </div>
      </div>
